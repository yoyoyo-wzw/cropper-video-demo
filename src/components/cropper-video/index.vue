    ...
    <div class="cropper-video">
        ...
        <div class="crop-filter">
            <div
                ...
                <div
                    :style="{
                        width: data.endLeft - data.startLeft - startSlideWidth + 'px',
                ...
                <div
                    ref="rightShade"
                    :style="{ width: Math.max(0, shaftWidth - data.endLeft - endSlideWidth) + 'px' }"
                    class="right-shade"
                />
                <div
                    ref="start"
        ...
        <div class="flex">
            <button
                type="success"
                size="small"
                @click="onSplitPlay">播放</button>
            <button
                :disabled="isStartFrame || isStartSplit"
                type="primary"
                size="small"
                @click="onStartCropper">确定裁剪</button>
        </div>
    </div>
</template>
<script>
...
export default {
    ...
    methods: {
        async updateFile () {
            if (!this.file) return;
            this.removeData();
            let { url: videoUrl, duration } = await getVideoInfo(this.file);
            ...
            // 设置滑块的开始结束位置
            const startSlideBlockRef = this.$refs.start;
            const endSlideBlockRef = this.$refs.end;
            startSlideBlockRef.style.left =
                startM / this.data.roal - this.startSlideWidth + 'px';
            endSlideBlockRef.style.left = endM / this.data.roal + 'px';

            this.data.startLeft = startSlideBlockRef.offsetLeft;
            this.data.endLeft = endSlideBlockRef.offsetLeft;
            this.timeList.push(getNowTime(tempTimeM));
            ...
            if (this.videoInfo.frames.length) {
        ...
        async onStartCropper () {
            ...
            let data = ffmpeg.FS('readFile', tempName); // 读取缓存
    ...
    mounted () {
        ...
        const startSlideBlockRef = this.$refs.start;
        this.startSlideWidth = startSlideBlockRef.clientWidth
        this.data.startLeft = -this.startSlideWidth

        const endSlideBlockRef = this.$refs.end;
        this.endSlideWidth = endSlideBlockRef.clientWidth
